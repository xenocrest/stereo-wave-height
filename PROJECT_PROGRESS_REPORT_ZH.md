...
本阶段的输入是空间点 P₍w₎=(X,Y,Z)ᵀ、相机姿态和镜头参数，输出是图像像素 p=(u,v)ᵀ。针孔相机模型写为
...
R、t 先把世界坐标点变换到相机坐标系，*K* 再把相机坐标投影到图像；fₓ、fᵧ 是以像素为单位的焦距，cₓ、cᵧ 是主点，*s* 是齐次尺度。若点已经位于相机坐标系中，公式展开为
...
切向畸变再由 p₁、p₂ 修正。因此可用于重建的标定结果不只有焦距，还必须包含左右内参 K左、K右、畸变参数 D左、D右 和左右相对姿态 *R、T*。
...
标定的输入是许多组 Pⱼ↔pᵢⱼ，未知量是相机内参、畸变以及每一帧棋盘相对相机的姿态。

### 3.2 单目标定的优化模型

对左相机，需要求 K左、D左 和每个棋盘姿态 R左ᵢ、t左ᵢ；右相机同理。模型用候选参数预测角点位置 p̂，并最小化真实检测位置与预测位置的差：
...
固定 K左、D左、K右、D右 后，联合观测与外参的求解关系为
...
项目比较四系数 (k₁,k₂,p₁,p₂) 与五系数 (k₁,k₂,p₁,p₂,k₃) 模型。增加 k₃ 能描述高阶径向畸变，但观测不足时也可能过拟合，所以不能只看训练 RMS。
...
若 eᵥ 仍有十几或几十像素，水平搜索的匹配器就很难找到真正对应点。HomeTank_004 的纵向 RMS 为 21.123 px，这一实验直接说明后续问题不能只归因于匹配参数，必须回到标定和校正几何。HomeTank_005 将其降至 13.045 px，但仍不够稳定。
...
所以工作距离越远，视差误差的影响按 Z² 放大；基线或像素焦距越大，深度对同样视差误差越不敏感。标定误差造成错误极线，匹配误差造成错误视差，两者最终都会传播到三维坐标和水面高度。
...
左右两幅图的联合约束通过线性或非线性三角化求得 X=(X,Y,Z,1)ᵀ。平行且已校正的理想情形与上一章的 Z=fB/d 一致；一般三角化则能处理完整投影矩阵。

### 5.2 WASS 在数学链中的位置

OpenCV 标定输出 K左、D左、K右、D右、R、T；WASS 使用这些几何信息和真实左右图像，寻找对应关系、计算稠密视差、三角化并输出点集
...
最大损失首先发生在匹配和有效三角化之前；连通分量筛选又移除 93,959 点，而平面裁剪没有继续丢点。由此得到 “匹配支持受限” 的阶段结论：数学深度公式已经明确，真正困难是低纹理、反光水面能否提供可靠左右对应。
...
这一层的输入是 XYZ、右相机内参和正确坐标变换，输出是 (u,v)↔(X,Y,Z)。

一次真实闭环中，WASS 已产生 98,438 个有限 XYZ，但最初投影结果却是 “直接观测数为 0”，与几何模型矛盾。逐步检查“XYZ—旋转—相机坐标—像素投影”后，定位到 WASS 相机点云中的行主序逆旋转在 Python 解码时被额外转置。修复后，98,438 个点均可投影回 1920×1080 图像、公共视场和所选 ROI。该实验验证的不只是“点云存在”，而是三维点确实能回到它所属的图像位置。

## 7. 从 XYZ 到水面高度：参考平面模型

### 7.1 为什么相机 *Z* 不能直接作为波高

相机通常倾斜拍摄，camera-frame *Z* 表示沿光轴方向的深度，并不等于垂直于平均水面的高度。同一个水平面上的点，也可能具有不同相机 *Z*。因此一般情况下不能定义 H=Z−Z₀。

### 7.2 参考平面拟合

静水参考点集记为 Pᵢ=(Xᵢ,Yᵢ,Zᵢ)ᵀ，参考平面写成
...
第一项是数据项，使估计曲面贴近真实 WASS 高度；第二项惩罚相邻网格的突然跳变；第三项惩罚不合理曲率和孤立尖峰。权重 wᵢ 允许可靠观测发挥更强约束。缺乏局部支持时，残差回到有限的基趋势，而不是无限发散。
...
合成实验不是只把数字代入公式，而是先生成左右相机真正可以读取的灰度图像，再让标定接口、WASS、网格化和高度模块按正式数据流运行。因为相机参数、水面方程和每个时刻的高度都预先已知，所以可将输出与 ground truth 逐项比较。它隔离了真实设备中的抖动、反光、曝光和标定误差，回答“数学链和软件实现本身是否自洽”。

### 9.1 统一虚拟相机与成像参数

主要合成实验使用 2448×2048 像素虚拟相机、3.45 μm 像元、8 mm 名义焦距、0.20 m 基线和 2.00 m 工作距离，并先采用零畸变理想模型。像素焦距由

$$
f_{px}=\frac{8\ \mathrm{mm}}{0.00345\ \mathrm{mm/px}}
=2318.8406\ \mathrm{px}
$$
...
$$
d=\frac{f_{px}B}{Z}
=\frac{2318.8406\times0.20}{2.00}
=231.8841\ \mathrm{px}.
$$

合成左右图像测得的最佳整数水平位移为 232 px，与理论值只相差 0.116 px。所有画面使用同一固定随机种子生成共享物理纹理，RIGHT 图像来自同一表面的第二视角投影，而不是复制 LEFT 图像后任意平移。

### 9.2 投影、视差与三角化闭环

第一组实验直接检查不同深度下的理论视差与虚拟图像视差。

| 深度 Z | 理论视差 | 虚拟相机视差 | 绝对差 |
|---:|---:|---:|---:|
| 1.50 m | 309.178744 px | 309.178744 px | 5.68×10⁻¹⁴ px |
| 2.00 m | 231.884058 px | 231.884058 px | 2.84×10⁻¹⁴ px |
| 2.50 m | 185.507246 px | 185.507246 px | 5.68×10⁻¹⁴ px |
| 3.00 m | 154.589372 px | 154.589372 px | 2.84×10⁻¹⁴ px |

随后使用左右投影像素独立三角化。4331 个平面点的三维 RMSE 为 9.51×10⁻¹⁶ m，4331 个正弦曲面点的三维 RMSE 为 9.71×10⁻¹⁶ m；最大误差分别为 2.01×10⁻¹⁵ m 和 2.57×10⁻¹⁵ m。误差处于浮点数舍入量级，说明投影矩阵、左右坐标方向、视差符号和三角化实现一致，可以进入真实 WASS 图像链。

### 9.3 Case 0：静水零高度实验

输入为两个独立编号、同一静水平面的合成双目帧。WASS 的 prepare、match、自动标定和 stereo 全部正常结束；每帧最终保留 4,313,574 个三维点。点云对齐到已知静水平面后的空间 RMSE 约 0.960 mm；进入正式规则网格后，对已知平面的 Z 误差 RMSE 为 0.554 mm。

| Case 0 指标 | 实验结果 |
|---|---:|
| 最终 XYZ | 每帧 4,313,574 点 |
| 直接点支持占整幅图像比例 | 86.039% |
| 规则网格有限覆盖率 | 100% |
| 高度 H 的 RMSE | 0.00446 mm |
| 高度 H 的 MAE | 0.00353 mm |
| 高度 H 的最大绝对误差 | 0.01228 mm |

这里极小的 H 误差主要反映同一确定性静水重建的数值重复性；不能把它解释为真实相机已经具备微米级测量精度。该实验真正证明的是：静水参考、共同有效域、规则网格和零高度符号能够闭环，因此下一步必须用非零高度检验，避免“自己减自己必然接近零”的假通过。

### 9.4 Case 1：+10 mm 固定高度实验

Case 1 使用两帧静水建立参考面，再输入两帧整体抬高 +10 mm 的平面。真值为

$$
H_{true}=+0.010\ \mathrm{m}.
$$

初始冻结配置虽然得到正的平均高度 8.889 mm，但 RMSE 为 11.080 mm、最大误差为 75.466 mm，未通过预设门限。诊断发现决定性损失发生在深度连通分量筛选。随后只对该理想合成案例执行单因素百分位实验；当百分位从 99 提高到 99.5 时，静水与抬高面的主要分量保留率分别达到 99.985% 和 99.890%，原始网格支持从约 51.45% 提升到 100%。
...
三次 WASS stereo 输出逐点完全一致；官方 DCT 网格器虽不是文件级完全相同，但跨运行最大 Z 差异仅 0.0206 mm，高度 RMSE 位于 1.0262～1.0274 mm。该实验验证了非零高度的尺度、正负号、参考面差分与数值重复性，并保留了初始失败结果，没有用适配结果覆盖失败证据。

### 9.5 Case 2：规则正弦波及幅频对比
...
基准组取振幅 10 mm、波长 0.80 m、频率 0.50 Hz；空间上每个波长有 80 个网格采样，时间上每周期有 10 个采样。最初评价得到 RMSE 5.397 mm，并出现 +0.7853 rad 相位差。进一步核对发现真实世界横坐标与输出网格原点相差 0.10 m，恰好对应 π/4 相位；显式采用已经记录的坐标关系后，相位误差降为 -0.000111 rad，诊断 RMSE 降为 0.862 mm。修正的是评价坐标原点，不是重写 WASS 高度。

随后只改变振幅和频率，形成四组受控实验：

| 组别 | 振幅／频率 | raw support | RMSE | MAE | 最大误差 | 恢复振幅 | 相位误差 | 结果 |
|---|---|---:|---:|---:|---:|---:|---:|---|
| G0 | 10 mm／0.50 Hz | 100% | 0.859 mm | 0.691 mm | 2.509 mm | 9.694 mm | +0.000207 rad | PASS |
| G1 | 30 mm／0.50 Hz | 100% | 1.030 mm | 0.849 mm | 3.398 mm | 29.596 mm | +0.000406 rad | PASS |
| G2 | 10 mm／1.00 Hz | 100% | 0.739 mm | 0.604 mm | 2.099 mm | 9.689 mm | -0.002633 rad | PASS |
| G3 | 30 mm／1.00 Hz | 100% | 1.131 mm | 0.925 mm | 3.672 mm | 29.659 mm | -0.003247 rad | PASS |

四组恢复波长均为 0.800 m，误差低于 10⁻¹² m；恢复频率误差均为 0 Hz。结果说明在已测试的振幅、频率和理想采样范围内，处理链能够保持波形的空间周期、时间周期和幅值趋势。它不是对任意高频波或真实重力波物理规律的证明。
...
$$
H(x,t)=\sum_{m=1}^{3}A_m\sin(k_mx-\omega_mt+\phi_m).
$$

三个分量的振幅为 15、8、5 mm，波长为 0.80、0.50、1.20 m，频率为 0.50、0.80、0.30 Hz，相位为 0、π/3、-π/4。理论高度严格位于 ±28 mm 内。原始 IRR-1 在 WASS 自动标定阶段稳定失败，因此保留为 FAIL，未制造后续高度。使用已有的十方向子集标定接口后，IRR-1A 才完成同一冻结波面的重建。
...
| 最大绝对误差 | 8.732 mm |
| P95 绝对误差 | 4.672 mm |
| 动态 raw support | 99.996%～100% |
| 最小分量保留率 | 99.750% |

结果说明同一数学链能够处理确定性的多频率叠加曲面，同时也暴露了少数空间点存在约 2.7～2.9 mm 正偏差。初始失败和局部偏差均被保留，因此结论仍限于理想合成图像。

### 9.7 基线与工作距离的工程参数实验

为了把公式 ∣δZ∣≈Z²∣δd∣/(fB) 转化为相机设计依据，项目分别改变工作距离和基线，其他条件保持一致。

| 工作距离 | 理论视差 | 理论灵敏度 | 实验结果 | 解释 |
|---:|---:|---:|---|---|
| 1.75 m | 265.010 px | 6.604 mm/px | RMSE 2.832 mm，但最小 raw support 71.758%，FAIL | 单帧支持塌缩 |
| 2.00 m | 231.884 px | 8.625 mm/px | RMSE 1.030 mm，raw support 100%，PASS | 冻结参考条件 |
| 2.50 m | 185.507 px | 13.477 mm/px | 平面拟合前终止，BLOCKED | 不生成虚假高度 |

距离实验没有证明“越近越好”或“2 m 最优”，而是说明几何灵敏度、公共视场和匹配支持必须共同满足。随后在固定 2.00 m 距离下改变基线：

| 基线 | 理论视差 | RMSE | MAE | 最大误差 | raw support | 结果 |
|---:|---:|---:|---:|---:|---:|---|
| 0.15 m | 173.913 px | 1.483 mm | 1.205 mm | 5.048 mm | 100% | PASS |
| 0.20 m | 231.884 px | 1.030 mm | 0.849 mm | 3.398 mm | 100% | PASS |
| 0.25 m | 289.855 px | 0.906 mm | 0.743 mm | 3.094 mm | 100% | PASS |

该受控区间内，增大基线改善了理论深度灵敏度和实测误差，但三角化点数随公共视场缩小而下降。因此不能简单选择最大基线。额外的 B=0.25 m、Z=2.50 m 交叉实验将名义视差恢复到 231.884 px，得到 RMSE 1.551 mm、MAE 1.222 mm、最大误差 8.306 mm并通过门限，说明 B/Z 比值是部署设计的重要变量，但尚不能替代完整二维参数扫描。

### 9.8 合成阶段的总体结论

合成阶段已经逐级验证：针孔投影与三角化达到机器精度；静水参考链能够得到零高度；+10 mm 平面能够恢复正确符号和毫米量级误差；规则波能够恢复振幅、波长、频率与相位；不规则波能够处理多分量叠加；基线和距离实验与理论灵敏度呈现一致趋势。

因此可以进入真实视频阶段，因为数学模型、坐标链和软件接口在“真值已知”的条件下已经闭环。与此同时，合成实验没有包含真实镜头畸变、标定不稳定、反光、曝光差、异步和机械位移，所以它证明的是算法正确性与参数趋势，不是现实水面的最终测量精度。
...
| 标定 | min Σ‖p−p̂‖² | 棋盘物点/像点 | K、D、R、T | HomeTank_004/005 | Stereo RMS 7.922→3.694 px |
| 极线 | x右ᵀFx左=0 | 标定与匹配角点 | 极线几何 | 004/005 | Epipolar RMS 9.508→3.708 px |
| 校正 | v左≈v右 | 双目几何 | 水平极线图像 | 004/005 | Vertical RMS 21.123→13.045 px |
| 视差 | d=u左−u右 | 校正图像对 | disparity | 搜索范围实验 | 扩大到 1280/2560 未解决支持 |
| 深度 | Z=fB/d | f、B、d | *Z* | 合成几何 | 已知深度闭环通过 |
| 三角化 | x×PX=0 | 左右对应与投影矩阵 | XYZ | HomeTank_004 WASS | 135,205 final XYZ，占公共视场 6.520% |
| 像素映射 | u=fₓX/Z+cₓ | XYZ、右相机模型 | pixel↔XYZ | 98,438 点闭环 | 重复转置修复后全部回到有效域 |
| 参考面 | nᵀP+d=0 | 静水 XYZ | n、d | 真实参考帧 | 101,548 XYZ，平面 RMS 29.923 mm |
...
4. 在真实几何稳定后，再迁移到同步专业双目相机，并用 |δZ|≈Z²|δd|/(fB) 反推基线、焦距、工作距离和所需视差精度。
...
| 相机内参 | K₁₁=fₓ，K₂₂=fᵧ，K₁₃=cₓ，K₂₃=cᵧ，K₃₃=1 | fₓ、fᵧ、cₓ、cᵧ | 描述投影尺度和主点，其余元素为 0 |
| 标定优化 | min Σ‖p−p̂‖² | K、D、Rᵢ、tᵢ | 让预测角点贴近检测角点 |
| 重投影 RMS | √(Σ‖p−p̂‖²/N) | 平均像素误差 | 衡量标定对观测的解释程度 |
| 左右外参 | P右=RP左+T | *R、T* | 连接左右相机坐标系 |
| 基线 | B=‖T‖ | *B* | 深度尺度来源 |
| 本质矩阵 | E=[T]×R | *E* | 只含双目相对姿态的极线几何 |
| 基础矩阵 | F=K右⁻ᵀEK左⁻¹ | *F* | 把极线约束表达在像素坐标中 |
| 极线约束 | x右ᵀFx左=0 | 对应点约束 | 缩小左右匹配搜索空间 |
| 视差 | d=u左−u右 | *d* | 表示对应点水平位移 |
| 深度 | Z=fB/d | *Z* | 把视差转换为距离 |
| 深度误差 | ∣δZ∣≈Z²∣δd∣/(fB) | 深度不确定度 | 指导焦距、基线和工作距离设计 |
| 三角化 | x×PX=0 | X、Y、Z | 由两条观测射线恢复三维点 |
| XYZ 回投影 | u=fₓX/Z+cₓ，v=fᵧY/Z+cᵧ | (u,v) | 建立像素—三维对应 |
| 参考平面 | aX+bY+cZ+d=0 | a、b、c、d | 定义零高度水面 |
| 平面拟合 | min Σ(nᵀPᵢ+d)²，‖n‖=1 | n、d | 从静水点云估计参考面 |
| 有符号高度 | H=(aX+bY+cZ+d)/√(a²+b²+c²) | *H* | 计算点到参考面的正交距离 |
| 曲面分解 | H=H基+δH | 基趋势与残差 | 分离稳定形状和局部变化 |
| 正则化曲面 | minₕ Σᵢwᵢ(hᵢ−Hᵢ观测)² + λ₁‖∇h‖² + λ₂‖Δh‖² | 连续 H(u,v) | 贴近观测并抑制跳变、尖峰与发散 |
