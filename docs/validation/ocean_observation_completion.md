...
> 后续坐标审计更正：WASS `computeP()` 写出的 `P0cam.txt` 不是 `rec_P1`，不能直接当成 rectified 像素投影。本页历史海面试验使用了这个未经验证的像素域约定，因此 **91.1746% 及其 ROI 对齐结论撤回为未验证**；相关性仅保留为旧坐标参数化下的内部数值实验，不是正确图像 ROI 的趋势证明。原始数组不删除。本轮 HomeTank_006 大范围批次改用源码明确的 computational-left XYZ → 实际 RIGHT 相机 → 原始畸变投影，独立统计，不沿用该数字。
...
参考法向必须统一朝上，不能每个 wave 帧重新置零；相机 Z 不能直接冒充水面高度。像素对应必须依据实际 WASS 相机交换和三角化坐标约定：将 computational-left XYZ 转换到目标相机，再用该相机内参与畸变投到 canonical 图像。不能把 `P0cam.txt` 当作 rectified 投影，也不能把 XYZ 包围盒拉伸成用户 ROI。以下历史试验的像素参数化尚未按此更正重新验证。
...
测量范围采用**用户预先框选**，不是按 WASS 结果选区域。现有 GUI 已将框选映射至 full canonical-cam1 多边形；新海面模式强制提供该多边形，缺失或 `observed_convex_hull` 均直接拒绝，不静默退回点云凸包。仅排除标定确定的非公共像素，记录用户框选像素数、非公共排除数、最终固定评价域、原始观测率与补全覆盖率。支持变少不改变分母；支持不足时报告失败，不能缩 ROI 提高百分比。建议用户一次框住画面中主要水面，避开岸边/槽壁及非水面物体，随后固定范围进行比较。本次不重打包 GUI，也不声称旧 EXE 已启用新模型。
