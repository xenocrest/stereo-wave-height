...
3. 新外参的米制尺度重新由已确认的 160 mm 棋盘跨度求出：B = 0.115797906 m；三个姿态的跨度尺度差 1.328 mm。它是标定推导量，不是人工测量 baseline，测量不确定度仍未知。
...
|---:|---:|---:|---:|---:|
| 1 | 35537 | -1.072 | 24082 | 6.338 |
| 2 | 36078 | -0.887 | 23736 | 6.742 |
| 3 | 32321 | -1.000 | 24311 | 6.438 |
| 10 | 16442 | -3.219 | 0 | UNAVAILABLE |
...
用户已确认：水面没有放东西，白色纹理为槽底痕迹；后续指定“槽底与静水面平行”为模型假设，无需另行验证。直接三角化不等于水面重建，当前转入折射成像模型；不能把槽底点补成水面高度。详见[折射链实际试验](refractive_height_probe.md)。
