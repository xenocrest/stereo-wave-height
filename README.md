...
面向汇报的完整中文阶段性总览见 [PROJECT_PROGRESS_REPORT_ZH.md](PROJECT_PROGRESS_REPORT_ZH.md)。

从双目视频、标定、同步、视差、三角化、静水参考到逐像素波浪高度的完整公式链见 [从双目视频到逐像素波浪高度：完整数学链路](MATHEMATICAL_WORKFLOW.md)。

当前模型层已形成双后端路线：WASS继续作为主重建引擎，同时新增标定驱动的OpenCV稠密双目备用后端；两者的可信观测可共同进入按物理水面坐标构造的观测锚定变分求解器，为最终全ROI逐像素高度服务。GUI暂时冻结，模型通过真值和独立物理验证后再统一接入。
