...
上述软约束形式保留为历史兼容模式。当前面向海面的新模式以基面 p 的残差 q=h-p 为未知量，直接施加严格观测约束：

$$
\min_q\;\lambda\|B_gq\|_2^2+\mu\|B_g^TB_gq\|_2^2,
\qquad q_i=h_i^{obs}-p_i\quad(i\in O).
$$

这样缺口求解使用的锚点与最终输出的观测完全相同，而不是软拟合完成后才覆盖观测值。每个 ROI 连通分量必须含直接双目锚点，并满足明确的观测覆盖门；否则拒绝生成。直接观测与模型估计仍分别标记。这里的物理距离算子是离散图正则化，参考平面足迹是空间近似，不宣称精确非平面光线交点或独立测量准确度。水槽底部纹理不能充当海面水面观测。实现和实测边界见[海面观测与补全验证](docs/validation/ocean_observation_completion.md)。
