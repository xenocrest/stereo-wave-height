...
**未解决诊断项。**约 45 deg 相位偏移是实质问题，当前证据尚不能区分 x 坐标原点/平面对齐、时间零点/帧定义或重建相位偏差。它没有被隐藏或事后修正，也不能因为高度门限通过就写成“相位问题已解决”。在进行相位敏感的科学使用和部署参数扫描前应先关闭该诊断。详见 [Case 2 报告](docs/validation/case2_sinusoidal_wave.md) 和冻结配置 [case2_sinusoidal.yaml](configs/simulation/case2_sinusoidal.yaml)。
...
1. **关闭 Case 2 相位/坐标/时间对齐诊断。**分别核对规则网格 x 原点、WASS 平面对齐变换、真值采样位置、动态首帧时间零点、时间戳映射和 Fourier/最小二乘相位定义；保持原始结果不变，不做事后相位修正。
2. **部署参数空间验证。**预注册并扫描 `baseline x scene distance`，同时检查公共视场、视差范围、三角化角度、raw support、误差和失效点；必要时再加入焦距、视场和波参数维度。单次仿真的 0.20 m x 2.00 m 不能替代参数空间结论。
3. **形成采购依据并采购设备。**结合候选设备规格、接口、同步方案、镜头准确型号、机械刚度、数据吞吐和参数扫描结果冻结物料清单。
4. **真实标定与同步验证。**分别标定两机内参/畸变，测量外参和真实基线，验证硬件触发、曝光时间、帧配对、时钟和漂移。
5. **真实静水 `Z0`。**在固定部署下采集独立静水序列，检查平面度、空间稳定性、时间重复性、有效域和环境光变化。
6. **人工波实验。**从已知固定高度/规则波开始，引入独立位移或波高参考，预注册 ROI 和门限，报告 bias、RMSE、MAE、max、覆盖及失败帧。
7. **扩展到真实海面。**按更大距离、风浪纹理、反光、遮挡、平台振动和环境变化重新建立误差预算及部署方案，并用现场独立参考重新验证；水槽结论不能直接外推。
