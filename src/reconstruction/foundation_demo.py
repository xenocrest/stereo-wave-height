...
def run(request):
    ...
    # FoundationStereo's disparity sign convention may differ from the
    # rectification Q convention; retain finite reprojected points in ROI and
    # let the geometric plane distance define height.
    valid=roi_model&np.isfinite(points).all(axis=2)
