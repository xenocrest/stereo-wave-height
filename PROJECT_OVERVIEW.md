...
测量交互现已加入 `automatic stereo common-FOV detection → safe water ROI selection`。该边界由 K/D/R/T、视频分辨率和生产 rectification policy 共同确定，不依赖图像内容猜测；更换标定或视频对会使公共视场、ROI 和参考面失效并重新建立。

## 1. 项目目标

当前按“成熟开源模型优先”推进：已复现 Fast-FoundationStereo 的实际推理、米制几何和独立参考窗口计算，并完成 HomeTank_004/005 与官方海面小样本对照。它是隔离候选，不代表逐像素准确度已经验收，也未替换冻结 GUI；[结果及下一步依据](docs/reconstruction/fast_foundationstereo_trial.md)分别记录估算覆盖与对应可靠性。

当前模型工作以海面适用性为约束：优先提高和验证 WASS 的真实水面观测支持，再用受这些观测严格约束的通用空间模型补全缺口，目标仍为公共水面 ROI 每个像素的高度与合理趋势。不得把水槽底部纹理三角化结果当成水面，也不以已知槽深/底纹作为海面部署条件。GUI 维持现状；新增模型先独立验证，再接入，不能用旧样例冒充所选视频帧。详见[海面模型与原分辨率试验](docs/validation/ocean_observation_completion.md)。
...
贯穿视频同步、相机模型、极线校正、双后端视差、三角化、pixel–XYZ、静水参考、全像素受约束曲面和误差传播的统一推导见 [完整数学链路](MATHEMATICAL_WORKFLOW.md)。当前模型路线不再把系统成败绑定于单一重建实现：WASS为主后端，OpenCV校正稠密双目为备用后端，二者均须经过相同几何与质量门；可靠观测随后约束物理坐标中的全像素变分高度场。该路线保持“公共水面ROI每个像素有高度”的最终目标，同时禁止无观测锚点的任意外推。
